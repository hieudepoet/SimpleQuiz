...
function QuizListPage() {
    ...
    const { quizzes, loading, error } = (0, hooks_1.useAppSelector)((s) => s.quiz);
    ...
    const handleDelete = async (id, title) => {
        if (!window.confirm(`Delete quiz "${title}"?`))
            return;
        dispatch((0, quizSlice_1.deleteQuiz)(id));
    };
    return (<div className="container py-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        {user?.admin && (<react_router_dom_1.Link to="/admin/quizzes/new" className="btn btn-success">
      ...
      {!loading && quizzes.length === 0 && (<div className="text-center py-5">
          <i className="bi bi-inbox" style={{ fontSize: '4rem', color: '#ccc' }}></i>
          <p className="text-muted mt-3 fs-5">No quizzes yet. {user?.admin && 'Create one!'}</p>
        </div>)}

      <div className="row g-4">
        {quizzes.map((quiz) => (<div key={quiz._id} className="col-md-6 col-lg-4">
            <div className="card h-100 border-0 shadow-sm" style={{ transition: 'transform 0.2s', cursor: 'pointer' }} onMouseOver={e => (e.currentTarget.style.transform = 'translateY(-4px)')} onMouseOut={e => (e.currentTarget.style.transform = 'translateY(0)')}>
              <div className="card-body d-flex flex-column p-4">
                ...
                <div className="d-flex gap-2">
                  <react_router_dom_1.Link to={`/quizzes/${quiz._id}`} className="btn btn-primary btn-sm flex-grow-1">
                    {user?.admin
                ? <><i className="bi bi-gear me-1"></i>Manage</>
                : <><i className="bi bi-play-fill me-1"></i>Take Quiz</>}
                  </react_router_dom_1.Link>
                  {user?.admin && (<>
                      <react_router_dom_1.Link to={`/admin/quizzes/${quiz._id}/edit`} className="btn btn-outline-warning btn-sm">
                        <i className="bi bi-pencil"></i>
                      </react_router_dom_1.Link>
                      <button className="btn btn-outline-danger btn-sm" onClick={() => handleDelete(quiz._id, quiz.title)}>
