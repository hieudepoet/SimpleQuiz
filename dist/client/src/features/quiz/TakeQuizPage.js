...
function TakeQuizPage() {
    ...
    const [prevQuizId, setPrevQuizId] = (0, react_1.useState)(undefined);
          ...
          <div className="col-md-6 text-center">
            <div className="card border-0 shadow-lg rounded-4 p-4">
              ...
              <div className="progress mb-3" style={{ height: '1.5rem' }}>
                <div className={`progress-bar fw-bold ${percent >= 70 ? 'bg-success' : percent >= 50 ? 'bg-warning' : 'bg-danger'}`} style={{ width: `${percent}%` }}>{percent}%</div>
              </div>
              ...
              <div className="text-start mt-4">
                ...
                {questions.map((q, i) => (<div key={q._id} className={`mb-2 p-3 rounded border ${answers[i] === q.correctAnswerIndex ? 'border-success bg-success bg-opacity-10' : 'border-danger bg-danger bg-opacity-10'}`}>
                    ...
                    <small>
                      ...
                      {answers[i] !== q.correctAnswerIndex && (<> · Correct: <span className="text-success fw-semibold">{q.options[q.correctAnswerIndex]}</span></>)}
                    </small>
                  </div>))}
              </div>
      ...
      <div className="row justify-content-center">
        <div className="col-md-7">
          ...
          <div className="card border-0 shadow-sm rounded-4">
            <div className="card-body p-4">
              ...
              <div className="d-flex flex-column gap-2">
                {question.options.map((opt, i) => (<button key={i} className={`btn text-start p-3 border rounded-3 ${selected === i ? 'btn-primary text-white' : 'btn-outline-secondary'}`} onClick={() => handleSelect(i)} style={{ transition: 'all 0.15s' }}>
          ...
          <div className="d-flex justify-content-end mt-4">
            {current < totalQ - 1 ? (<button className="btn btn-primary px-4" onClick={handleNext} disabled={selected === null}>
                Next <i className="bi bi-arrow-right ms-1"></i>
              </button>) : (<button className="btn btn-success px-4 fw-semibold" onClick={handleSubmit} disabled={selected === null}>
