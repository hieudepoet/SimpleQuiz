...
export default function AuthPage() {
  ...
  const resetForm = () => {
    setUsername(''); setPassword(''); setIsAdmin(false); setAdminCode(''); setAdminCodeError('')
  }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (login.fulfilled.match(result)) navigate('/quizzes')
  }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const result = await dispatch(signup({ username, password, admin: isAdmin }))
  ...
  const switchTab = (t: 'login' | 'signup') => {
    setTab(t); dispatch(clearError()); setSuccessMsg(''); resetForm()
  }

  return (
    <div className="min-vh-100 d-flex align-items-center" style={{ background: 'linear-gradient(135deg,#1e3a8a,#3b82f6)' }}>
              ...
              <div className="card-body p-4">
                ...
                <ul className="nav nav-tabs mb-4 border-0" role="tablist">
                  <li className="nav-item flex-fill text-center">
                    <button
                      className={`nav-link w-100 fw-semibold ${tab === 'login' ? 'active text-primary border-bottom border-primary border-2' : 'text-muted'}`}
                  ...
                  <li className="nav-item flex-fill text-center">
                    <button
                      className={`nav-link w-100 fw-semibold ${tab === 'signup' ? 'active text-primary border-bottom border-primary border-2' : 'text-muted'}`}
                    ...
                    <div className="mb-4">
                      <div className="form-check mb-2">
                        <input
                          ...
                          id="isAdminCheck"
                          checked={isAdmin}
                          onChange={(e) => { setIsAdmin(e.target.checked); setAdminCode(''); setAdminCodeError('') }}
                        />
                      ...
                      {isAdmin && (
                        <div>
                          <input
                            type="password"
                            className={`form-control ${adminCodeError ? 'is-invalid' : ''}`}
                  ...
                  <div className="d-grid">
                    ...
                    >
                      {loading ? (
                        ...
                        <><i className={`bi ${isAdmin ? 'bi-shield-check' : 'bi-person-plus'} me-2`}></i>
